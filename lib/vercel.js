...
// Busca deployments paginando (a API devolve no máx 100 por página).
// Segue o cursor pagination.next para montar um histórico maior.
async function fetchDeployments(cfg, teamId, max = 300) {
  const out = [];
  let until;
  let guard = 0;
  while (out.length < max && guard < 10) {
    guard++;
    let path = '/v6/deployments?limit=100';
    if (until) path += `&until=${until}`;
    const data = await vercelFetch(path, cfg, teamId);
...
module.exports = { API, getConfig, vercelFetch, listScopes, fetchDeployments, sendError };
